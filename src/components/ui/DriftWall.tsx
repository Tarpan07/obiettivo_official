...
const DriftWall = ({
  ...
  // Global mouse move tracker for decorative backgrounds
  useEffect(() => {
  ...
  const tileClass = cx(
    ...
    decorative && "pointer-events-none cursor-default"
  );
  const innerClass = cx(
    ...
    !decorative && "group-[.is-active]/tile:opacity-100 group-[.is-active]/tile:[transform:translateZ(var(--dw-lift))]",
    !decorative && "group-[.is-active]/tile:shadow-[0_24px_60px_-18px_rgba(0,0,0,0.7)]",
    !decorative && "group-focus-visible/tile:opacity-100 group-focus-visible/tile:[transform:translateZ(var(--dw-lift))]",
    !decorative && "group-focus-visible/tile:shadow-[0_24px_60px_-18px_rgba(0,0,0,0.7),0_0_0_2px_rgba(255,255,255,0.9)]"
  );
  const imgClass = cx(
    ...
    !decorative && "group-[.is-active]/tile:[filter:grayscale(0)_saturate(1.05)]",
    !decorative && "group-focus-visible/tile:[filter:grayscale(0)_saturate(1.05)]"
  );
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    ...
    const commonProps = {
      className: cx(tileClass, !decorative && activeId === id && "is-active"),
      "data-tile-id": id,
      "data-col": colIndex,
    ...
    return (
      <div
        ...
        aria-label={decorative ? undefined : (item.title ?? "tile")}
        {...commonProps}
      >
        {inner}
      </div>
